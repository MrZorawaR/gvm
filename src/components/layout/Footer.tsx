...
export default function Footer({ footerLinksData = footerLinks }) {
  return (
    <footer className="w-full py-8 bg-accent-bg text-accent-text">
      {/* Row 1: Links */}
      <div className="flex flex-wrap justify-center mb-6">
        {footerLinksData.map((section, index) => (
          <div key={index} className="flex justify-center px-6 mb-4">
            <ul className="list-none p-0 m-0 text-left">
              <li className="mb-2 font-semibold">{section.category}</li>
              {section.links.map((link, i) => (
                <li key={i}>
                  {link.path ? (
                    <Link
                      to={link.path}
                      className="text-accent-text no-underline hover:opacity-75 transition-opacity text-sm"
                    >
                      {link.pathname}
                    </Link>
                  ) : (
      ...
      <div className="px-8 mb-4">
        ...
        <div className="flex gap-3">
          <a href="#" className="text-accent-text hover:opacity-75 transition-opacity">
          ...
          <a href="https://www.instagram.com/kualakubsworldschool" className="text-accent-text hover:opacity-75 transition-opacity">
            <svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" fill="currentColor" viewBox="0 0 16 16">
